...
// test("useVisualMode should return to previous history", () => {
//   const { result } = renderHook(() => useVisualMode(FIRST));

//   act(() => result.current.transition(SECOND));
//   expect(result.current.history).toBe([FIRST, SECOND]);

//   act(() => result.current.transition(THIRD));
//   expect(result.current.history).toBe([FIRST, SECOND, THIRD]);
